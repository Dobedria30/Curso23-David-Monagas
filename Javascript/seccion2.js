//Funciones
/*
  Las funciones son bloques de condigo que realizan una tarea especificay
...
function holaMundo() {
  ...
  holaMundo(); // Esto ejecuta la funcion y muestra "Hola, mundo!" en la consola
...
saludar(1); // Esto muestra "¡Hola, María!" en la consola
saludar(true); // Esto muestra "¡Hola, Pedro!" en la consola
...
sumar(5, 10); // Esto muestra 15 en la consola
...
for (let i = 0; i <= 100; i += 5) { // for (let i = 0; i < motos.length; i++)
...
for (let numero of números) {
  console.log( "resultados:" + numero * multiplo);
}
...
for (let verdes = 0; verdes < Supermercado.verduras.length; verdes++){
  console.log(Supermercado.verduras[verdes])
}
...
for (let compras in SuperM){
  console.log(compras + ": " + SuperM[compras])
}
